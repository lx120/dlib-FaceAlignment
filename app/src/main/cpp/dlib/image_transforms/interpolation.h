...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        {
            y += y_scale;        //r=0时，y=0
            const long top    = static_cast<long>(std::floor(y));        //std::floor(y)向下取整，返回为浮点型,static_cast<long>()为类型转换为long型，向下取整
            const long bottom = std::min(top+1, in_img.nr()-1);
            const double tb_frac = y - top;
            double x = -4*x_scale;
...
// ----------------------------------------------------------------------------------------
    ...
    {
        // make sure requires clause is not broken	确保要求的条款不被破坏
        DLIB_ASSERT( images.size() == objects.size(),
            "\t void upsample_image_dataset()"
            << "\n\t Invalid inputs were given to this function."
            << "\n\t images.size():   " << images.size() 
