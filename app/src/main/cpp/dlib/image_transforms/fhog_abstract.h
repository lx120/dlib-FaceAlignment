    ...
    /*!
        ...
        ensures
            - This function is identical to the above extract_fhog_features() routine
              except that it outputs the results in a planar(二维的) format rather than the
              interlaced(交错的) format used above.  That is, each element of the hog vector is
    ...
    /*!
        ...
        ensures
            ...
            - If H is the array<array2d<double>> object output by the planar
              ...
              H[0], then H[1], then H[2], and so on in ascending(提升) index order.
    !*/
    ...
    /*!
        ...
        ensures
            - Maps a pixel in a FHOG image (produced by extract_fhog_features()) back to the
              ...
              spatially downsampled by aggregation(聚合) into cells the mapping is not totally
              invertible(可逆的).  Therefore, the returned location will be the center of the cell
...
// ----------------------------------------------------------------------------------------
    ...
    /*!
        requires
            ...
            - hog.size() == 31
        ensures
            - Interprets(解释) hog as a FHOG feature map output by extract_fhog_features() and
              converts(转换) it into an image suitable for display on the screen.  In particular,
              we draw all the hog cells into a grayscale image in a way that shows the
              magnitude(大小) and orientation of the gradient energy in each cell.  The result is
              then returned.
            - The size of the cells in the output image will be rendered(呈现) as cell_draw_size 
