...
namespace dlib
{
    ...
    public:
        ...
        inline simd4f() {}
        inline simd4f(float f) { x = _mm_set1_ps(f); }      //给128位寄存器分配4个float，每个float的值均为f
        inline simd4f(float r0, float r1, float r2, float r3) { x = _mm_setr_ps(r0,r1,r2,r3); }
        inline simd4f(const __m128& val):x(val) {}
        inline simd4f(const simd4i& val):x(_mm_cvtepi32_ps(val)) {}    //val的 32 位整数值设置为单精度浮点值。
    ...
    private:
        __m128 x;             //__m128（4个float）、__m128d（2个double）、__m128i(int、short、char)直接控制那些128-bit的寄存器。
    };
