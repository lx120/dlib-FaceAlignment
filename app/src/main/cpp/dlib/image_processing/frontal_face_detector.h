...
namespace dlib
{
    typedef object_detector<scan_fhog_pyramid<pyramid_down<6> > > frontal_face_detector;          //基于目标检测定义一个正脸检测器
    inline const std::string get_serialized_frontal_faces();                                      //获取序列化的正脸
	//inline 定义的类的内联函数，函数的代码被放入符号表中，在使用时直接进行替换，（像宏一样展开），没有了调用的开销，效率也很高

    inline frontal_face_detector get_frontal_face_detector()
    {
        std::istringstream sin(get_serialized_frontal_faces());        //获取序列化的正脸？？？？？是不是加载训练好的人脸检测参数？？？？
        frontal_face_detector detector;
        deserialize(detector, sin);                                    //反序列化\并行化
        return detector;
    }

// ----------------------------------------------------------------------------------------
	/*
		它使用5个HOG过滤器建立：正面的视角、左面视角、右面视角、人脸向左偏转的正面视角、人脸向右偏转的正面视角

		同时，下面是训练日志以及用来产生过滤器的参数：
		正面的检测器：
			在labeled_faces_in_the_wild/frontal_faces.xml镜像集合中训练
			通过2:1的比例对每个图像进行向上采样，即图像扩大2倍
			使用6层图像金字塔
			每个失误目标的损失：1
			ε：0.05
			填充：0
			...
			过滤器数目：78
			图片数目：4748
			训练检测器(精度，召回率，AP):0.999793 0.895517 0.895368
			单个阈值：0.15
	*/
    ...
    */
    inline const std::string get_serialized_frontal_faces()
    {
		...
		// base64 从我们想要解码和返回的文件object_detector.dat中编码数据
