...
// ----------------------------------------------------------------------------------------
    ...
    {
        double detection_confidence;   //置信度
        unsigned long weight_index;    //权值索引
        rectangle rect;                //矩形框

        bool operator<(const rect_detection& item) const { return detection_confidence < item.detection_confidence; }
		//运算符重载"<"，判断本rect_detection的detection_confidence小于item.detection_confidence的话返回true，否则返回false
    };
...
// ----------------------------------------------------------------------------------------
    ...
    template <typename image_scanner_type>   //图像扫描类型
    struct processed_weight_vector
    {
        processed_weight_vector(){}           //构造函数？？？？？？？

		typedef typename image_scanner_type::feature_vector_type feature_vector_type;       //特征向量类型
        ...
        /*!
			...
			要求
				- w 已经被分配了值。注意这个函数里面的点是为了允许一个图像扫描器重载processed_weight_vector模板并且提供一些不同类型的目标作为get_detect_argument()的输出。
				例如，scan_fhog_pyramid目标使用一个重载，这造成了get_detect_argument()返回具体的fhog_filterbank目标而不是feature_vector_type。
				这避免了在每次调用detect检测器的时候需要去构造fhog_filterbank类型，因此加速了检测。
        !*/
        {}

        // return the first argument to image_scanner_type::detect()
		// 返回第一个参数到image_scanner_type::detect()
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        typedef typename image_scanner_type::feature_vector_type feature_vector_type;         //typedef typename表明feature_vector_type是一种类型
        ...
        unsigned long num_detectors (             //在该类末尾定义的有std::vector<processed_weight_vector<image_scanner_type> > w;
        ) const { return w.size(); }              //返回类别数

        const feature_vector_type& get_w (        //返回该类的特征向量，应该是filter
            unsigned long idx = 0
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        std::vector<feature_vector_type> weights;      //全部检测器的全部特征向量
...
// --------------------------主要进行检测的函数--------------------------------------------------------------
    ...
    {
        scanner.load(img);           //image_scanner_type  scanner
        std::vector<std::pair<double, rectangle> > dets;           //double是置信度
        std::vector<rect_detection> dets_accum;
        for (unsigned long i = 0; i < w.size(); ++i)   //w.size() = 5, 权值向量w, filter??
        {
            const double thresh = w[i].w(scanner.get_num_dimensions());           // scanner.get_num_dimensions()返回所有金字塔的总共的cell个数
			// 检测目标，
			// image_scanner_type scanner;       image_scanner_type::feature_vector_type
            scanner.detect(w[i].get_detect_argument(), dets, thresh + adjust_threshold);
            for (unsigned long j = 0; j < dets.size(); ++j)
            {
                rect_detection temp;
                temp.detection_confidence = dets[j].first-thresh;       //置信度
                temp.weight_index = i;                                  //保存是在第几层金字塔检测出来的目标物
                temp.rect = dets[j].second;                             //保存目标边框：x,y,w,h
        ...
        final_dets.clear();
        if (w.size() > 1)
            std::sort(dets_accum.rbegin(), dets_accum.rend());     //升序排序，rbegin() 返回一个逆序迭代器，它指向容器c的最后一个元素，rend() 返回一个逆序迭代器，它指向容器c的第一个元素前面的位置
