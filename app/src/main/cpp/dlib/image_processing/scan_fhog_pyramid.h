...
// ----------------------------------------------------------------------------------------
    ...
    private:
        ...
        {
            const rectangle rect = centered_rect(point(0,0),window_width,window_height);  //80*80,初始时是64*64,  centered_rect为向左上方移动window_width/2和window_height/2....把窗口项为做向上采样是的坐标上投影？？
            const rectangle temp = grow_rect(fe.image_to_feats(rect, cell_size, 1, 1), padding);         //feature_extractor_type fe; array<fhog_image> feats;   按padding的规模扩大向四周rect的面积
			//image_to_feats()返回cell个数,两个point形式组成rectangle
            width = temp.width();
            height = temp.height();
        }
    ...
    namespace impl
    {
        ...
        )
        {
            ...
            if (feats.size() > 1)
            {
                ...
                array2d<pixel_type> temp1, temp2;     //二维像素矩阵
                pyr(img, temp1);
...
// ----------------------------------------------------------------------------------------
    ...
    {
        unsigned long width, height;
        compute_fhog_window_size(width,height);          // 返回cell在行和列上的个数，rectangle
        return width*height*fe.get_num_planes();         // fe.get_num_planes()为固定值31
    }
...
// ----------------------------------------------------------------------------------------
    ...
    namespace impl
    {
        ...
        ) 
        {
            ...
            array2d<float> saliency_image;           // 得分
            pyramid_type pyr;

			//ofstream fout("E:/FDDB/data2/score.txt", ofstream::out | ofstream::app);
            ...
            for (unsigned long l = 0; l < feats.size(); ++l)
            {
                ...
                // now search the saliency(显著；卓越；特点；凸起) image for any detections
                for (long r = area.top(); r <= area.bottom(); ++r)
                {
                    for (long c = area.left(); c <= area.right(); ++c)
                    {
                        ...
                        if (saliency_image[r][c] >= thresh)
                        {
							...
							//fout << saliency_image[r][c] - thresh << endl;
                        }
                    }
                }
            }
